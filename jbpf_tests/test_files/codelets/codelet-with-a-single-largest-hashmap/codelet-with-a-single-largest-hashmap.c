...
× 1024 entries (approximately 67 million), the raw value storage would require about 256 MB. However, the ck_ht hash map
...
struct jbpf_load_map_def SEC("maps") map1 = {
    ...
    // but the ck_ht implementation allocates 8x that amount, so total allocation is 512MB
};

/*
 * The purpose of this test is to ensure that an attempt to load too many codelets will fail.
 *
 * This test does the following:
 * 1. It loads 13 codeletSets, with each codeletSet having 5 codelets, giving a total of 65 codelets.
 * 2. The maximum codelets that can be loaded is 64.  Therefore the last codeleSet should fail to be loaded.
 * 3  The code asserts that the first 12 codeletSets are loaded successfully and the 13th codeletSet fails to load.
 * 4. The 13th codeletSet should fail to be laoded.  However the first 4 codelets of this codeletSet should be laoded
 *    OK, and the failure would occur when the 5th codelet is being processed.   The code also checks the jbpf context
 *    stats to be sure that loaded maps and codelets are cleand up properly in this case.
 * 5. The code then unloads all 13 codeletSets.  It asserts that the first 12 codeletSets are unloaded successfully and
 *    the 13th codeletSet fails to unload.
 * 6. The jbpf context stats are checked to ensure that all codelets and maps counts are zero at the end of the test.
 */

/*
 * The purpose of this test is to ensure we can configure an "out" channel with the maximum Serde length, it passes
 ...
 * 2. It populates the Serde of the out_io_channel with a string which is maximum length.
 * 3. It asserts that the return code from the jbpf_codeletset_load call is JBPF_CODELET_PARAM_INVALID, and that the
 * string starts with "out_io_channel.serde does not exist".
 */
...
main(int argc, char** argv)
{
    ...
    for (int i = 0; i < JBPF_PATH_LEN; i++) {
        codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].serde.file_path[i] = 'A'; // Non-zero ASCII value
    }
    codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].serde.file_path[JBPF_PATH_LEN - 1] = '\0';
    ...
    assert(
        strncmp(
            err_msg.err_msg, "out_io_channel.serde does not exist", strlen("out_io_channel.serde does not exist")) ==
        0);

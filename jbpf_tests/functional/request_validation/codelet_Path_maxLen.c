/*
 * The purpose of this test is to ensure that if an codelet path is configured with the maximum length, it passes
 * validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It explicitly sets codelet_descriptor[0].codelet_path to a string of the max length.
 * 3. The "load" request will fail.  However the test asserts that the return code is JBPF_CODELET_CREATION_FAIL.  This
 * means that it has passed the validation of the field length.
 */
...
main(int argc, char** argv)
{
    ...
    codeletset_req_c1.codelet_descriptor[0].codelet_path[JBPF_PATH_LEN - 1] = '\0';

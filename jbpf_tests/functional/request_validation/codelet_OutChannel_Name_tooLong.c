/*
 * The purpose of this test is to ensure we can configure an "out" channel but set name too long, it fails validation
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet with a single output channel.
 * 2. It populates the name of the out_io_channel with a string which is too long.
...
main(int argc, char** argv)
{
    ...
    for (int i = 0; i < JBPF_IO_CHANNEL_NAME_LEN; i++) {
        codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].name[i] = 'A'; // Non-zero ASCII value
    }

/*
 * The purpose of this test is to ensure that if a codelet name is set to the maximum length, that this is handled OK.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It explicitly sets the codelet name with a string which is maximum length.
...
main(int argc, char** argv)
{
    ...
    // Fill the codelet_name with non-zero values (e.g., 'A'), but terminate with NULL
    for (int i = 0; i < JBPF_CODELET_NAME_LEN; i++) {
        codeletset_req_c1.codelet_descriptor[0].codelet_name[i] = 'A'; // Non-zero ASCII value
    }
    codeletset_req_c1.codelet_descriptor[0].codelet_name[JBPF_CODELET_NAME_LEN - 1] = '\0';

/*
 * The purpose of this test is to ensure we can configure an "out" channel with the maximum name length, it passes
 * validation
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet with a single output channel.
 * 2. It populates the name of the out_io_channel with a string which is maximum length.
...
main(int argc, char** argv)
{
    ...
    strcpy(codeletset_req_c1.codeletset_id.name, "simple_output_shared_maxLen_codeletset");
    ...
    snprintf(
        codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].name,
        JBPF_MAP_NAME_LEN,
        "output_map_"
    ...
    if (!jbpf_path) {
        snprintf(
            ...
            JBPF_PATH_LEN,
            "../../jbpf_tests/test_files/codelets/simple_output_shared_maxLen/simple_output_shared_maxLen.o");
    } else {
        snprintf(
            codeletset_req_c1.codelet_descriptor[0].codelet_path,
            JBPF_PATH_LEN,
    ...
    strcpy(codeletset_req_c1.codelet_descriptor[0].codelet_name, "simple_output_shared_maxLen");

/*
 * The purpose of this test is to ensure if you specify more linked maps than is populated, and it is captured during
 * validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a two codelets (C1).
 * 2. For codelet 2, its set num_linked_maps to 3, but only populates 2.
...
main(int argc, char** argv)
{
    ...
    for (uint8_t m = 0; m < (codeletset_req_c1.codelet_descriptor[1].num_linked_maps - 1); m++) {

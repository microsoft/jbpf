/*
 * The purpose of this test is to ensure if, for a codelet, there are duplicate map_name, this fails validation checks.
 *
 ...
 * shared maps.
 * 3. Codelet 2 is configured with 2 linked maps, but both map_name are "shared_map_output0".
...
main(int argc, char** argv)
{
    ...
    for (uint8_t m = 0; m < codeletset_req_c1.codelet_descriptor[1].num_linked_maps; m++) {
        ...
        // Use the same map_name.  This should fail.
        strcpy(codeletset_req_c1.codelet_descriptor[1].linked_maps[m].map_name, "shared_map_output0");

/*
 * The purpose of this test is to ensure if, for a codelet, there are duplicate linked_map_name, this fails validation
 * checks.
 *
 ...
 * shared maps.
 * 3. Codelet 2 is configured with 2 linked maps, but both linked_map_name are "shared_map0".
...
main(int argc, char** argv)
{
    ...
    if (!jbpf_path) {
        snprintf(
            ...
            JBPF_PATH_LEN,
            "../../jbpf_tests/test_files/codelets/max_input_shared/max_input_shared.o");
    } else {
        snprintf(
            codeletset_req_c1.codelet_descriptor[0].codelet_path,
            JBPF_PATH_LEN,
    ...
    codeletset_req_c1.codelet_descriptor[1].num_linked_maps = JBPF_MAX_LINKED_MAPS;
    for (uint8_t m = 0; m < codeletset_req_c1.codelet_descriptor[1].num_linked_maps; m++) {
        ...
        // Use the same linked_map_name.  This should fail
        strcpy(codeletset_req_c1.codelet_descriptor[1].linked_maps[m].linked_map_name, "shared_map0");
    }
    ...
    if (!jbpf_path) {
        snprintf(
            ...
            JBPF_PATH_LEN,
            "../../jbpf_tests/test_files/codelets/max_output_shared/max_output_shared.o");
    } else {
        snprintf(
            codeletset_req_c1.codelet_descriptor[1].codelet_path,
            JBPF_PATH_LEN,

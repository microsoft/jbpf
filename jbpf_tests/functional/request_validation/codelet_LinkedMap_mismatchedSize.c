/*
 * The purpose of this test is to ensure if we can configure a linked map to use an map which has a different
 * definition, the load request will fail.
 *
 * This test does the following:
 * 1. It loads a codeletSet with two codelets.
 * 2. Codelet 1 is configured with file codelets/simple_input_largeShared/simple_input_largeShared.o.  This has one
 * input channel ("input_map"), and one shared channel ("shared_map").  The "shared_map" has 1000 entries.
 * 3. Codelet 2 is configured with file codelets/simple_output_shared/simple_output_shared.o.  This has one output
 * channel
 * ("output_map"), and one shared channel ("shared_map_output").  The "shared_map_output" has 1 entry.
 * 4. Codelet 2 is configured with 1 linked map.  It links "shared_map_out" to "shared_map".  This should
 * fail because, even though the map types are the same, the the sizes are different.
 * 5. The code asserts that the load request will fail with return code JBPF_CODELET_CREATION_FAIL.
 */
...
main(int argc, char** argv)
{
    ...
    snprintf(
        ...
        JBPF_PATH_LEN,
        "%s/jbpf_tests/test_files/codelets/simple_input_largeShared/simple_input_largeShared.o",
        jbpf_path);
    strcpy(codeletset_req_c1.codelet_descriptor[0].codelet_name, "simple_input_largeShared");
    ...
    strcpy(codeletset_req_c1.codelet_descriptor[1].linked_maps[0].map_name, "shared_map_output");
    strcpy(codeletset_req_c1.codelet_descriptor[1].linked_maps[0].linked_codelet_name, "simple_input_largeShared");

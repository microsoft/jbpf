/*
 * The purpose of this test is to ensure we can configure an "out" channel but dont set name, it fails validation
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet with a single output channel.
 * 2. It populates the name of the out_io_channel with a string which is zero length.
...
main(int argc, char** argv)
{
    ...
    codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].name[0] = '\0';

/*
 * The purpose of this test is to ensure that if an unknown codelet path is configured, it is captured during validation
 * checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It explicitly sets codelet_descriptor[0].codelet_path to a non-existent file.
...
main(int argc, char** argv)
{
    ...
    snprintf(
        ...
        JBPF_PATH_LEN,
        "%s/jbpf_tests/test_files/codelets/simple_output/unknown.o",
        jbpf_path);

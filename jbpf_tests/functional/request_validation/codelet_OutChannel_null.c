/*
 * The purpose of this test is to ensure if you specify more output channels than is populated, and it is captured
 * during validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It set num_out_io_channel to 3, but only populates 2.
...
main(int argc, char** argv)
{
    ...
    codeletset_req_c1.codelet_descriptor[0].num_out_io_channel = JBPF_MAX_IO_CHANNEL;

    for (int outCh = 0; outCh < (codeletset_req_c1.codelet_descriptor[0].num_out_io_channel - 1); outCh++) {

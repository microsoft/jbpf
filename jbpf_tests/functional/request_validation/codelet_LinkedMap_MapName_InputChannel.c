/*
 * The purpose of this test is to ensure that if we can configure a linked map to be an input map, the load request
 * will fail.
 *
 * This test does the following:
 * 1. It loads a codeletSet with two codelets.
 * 2. Codelet 1 is configured with file codelets/simple_input_shared/simple_input_shared.o.  This has one input channel
 ...
 * 4. Codelet 1 is configured with 1 linked map.  The "map_name" is set to "input_map".  This should cause the
 * load to fail.
...
main(int argc, char** argv)
{
    ...
    codeletset_req_c1.codelet_descriptor[0].num_linked_maps = 1;
    strcpy(codeletset_req_c1.codelet_descriptor[0].linked_maps[0].map_name, "input_map");
    strcpy(codeletset_req_c1.codelet_descriptor[0].linked_maps[0].linked_codelet_name, "simple_output_shared");

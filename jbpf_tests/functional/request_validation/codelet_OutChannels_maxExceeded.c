/*
 * The purpose of this test is to ensure that if too many output channels are configured, it is captured during
 * validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It populates the JBPF_MAX_IO_CHANNEL number of out_io_channel.
 * 3. It asserts sets num_out_io_channel to "JBPF_MAX_IO_CHANNEL + 1" i.e. configuring too many.
...
main(int argc, char** argv)
{
    ...
    for (int outCh = 0; outCh < JBPF_MAX_IO_CHANNEL; outCh++) {

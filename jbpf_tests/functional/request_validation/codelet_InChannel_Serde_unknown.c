/*
 * The purpose of this test is to ensure we can configure an "in" channel but set Serde too long, it fails validation
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet with a single input channel.
 * 2. It populates the Serde of the in_io_channel with a string which is too long.
...
main(int argc, char** argv)
{
    ...
    strcpy(codeletset_req_c1.codelet_descriptor[0].in_io_channel[0].serde.file_path, "unknown");

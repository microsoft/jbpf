/*
 * The purpose of this test is to ensure that if an unknown hook is configured, it is captured during validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It explicitly sets codelet_descriptor[0].hook_name to a non-existent hook.
 * 3. It asserts that the return code from the jbpf_codeletset_load call is JBPF_CODELET_HOOK_NOT_EXIST).
 */
...
main(int argc, char** argv)
{
    ...
    assert(res == JBPF_CODELET_HOOK_NOT_EXIST);

/*
 * The purpose of this test is to ensure if you specify more input channels than is populated, and it is captured during
 * validation checks.
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet (C1).
 * 2. It sets num_in_io_channel to 5, but only populates 4.
...
main(int argc, char** argv)
{
    ...
    for (int inCh = 0; inCh < (codeletset_req_c1.codelet_descriptor[0].num_in_io_channel - 1); inCh++) {

/*
 * The purpose of this test is to ensure we can configure an "out" channel but set Serde too long, it fails validation
 *
 * This test does the following:
 * 1. It creates the config for a codeletset with a single codelet with a single output channel.
 * 2. It populates the Serde of the out_io_channel with a string which is too long.
...
main(int argc, char** argv)
{
    ...
    strcpy(codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].name, "output_map_0");

    codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].has_serde = true;
    // set an unknown serde
    strcpy(codeletset_req_c1.codelet_descriptor[0].out_io_channel[0].serde.file_path, "unknown");
